...
;   and store/check the received 8-byte packet over  UART at 9600 baut for a valid 
...
;
;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~;
;TODO: Fully debugged 5/10
;- test the removal of queryStatus
...
#define QUERY_MASK      0x01    ;Mask the query bit in outputStatus
...
#define INTACT_AA       0x48    
#define INTACT_BB       0x75
#define INTACT_CC       0x6D
#define INTACT_DD       0x61
#define INTACT_EE       0x6E
#define POPPED_SUM      0x53    ;Correct Packet Data for popped balloon
#define POPPED_AA       0x5A
#define POPPED_BB       0x6F
#define POPPED_CC       0x6D
#define POPPED_DD       0x62
#define POPPED_EE       0x69
...
Main_Loop:
    CALL        EC_DataEvent    ;Checks and handles valid data packet events
    GOTO        Main_Loop

; Event Checkers (EC_)
EC_DataEvent:
    ;Check for intact or popped events
    BTFSC       dataEvent,0     ;If 1, validate packet against intact table
    GOTO        IntactTest      ;
    BTFSC       dataEvent,1     ;If 1, validate packet against popped table
    GOTO        PoppedTest      ;
...
IntactTestLoop:
    ...
    RETURN                      ;Return if packet doesn't match exactly
    ...
    GOTO        IntactConfirmed ;
    INCF        dataIndex       ;Increment dataIndex
    GOTO        IntactTestLoop  ;Loop
IntactConfirmed:
    ;Set output pin high since we have an intect balloon
    MOVF        PORTC,W         ;Read
...
PoppedTestLoop:
    ...
    ADDWF       dataIndex,W     ;Index to our current data byte
    MOVWF       FSR             ;Set FSR to current data byte address
    MOVF        INDF,W          ;Move the referenced data byte to 
    MOVWF       dataBuffer      ;Move the new data byte to the current data byte address
    ;Address correct byte for comparison
    MOVF        dataIndex,W     ;
    CALL        PoppedData      ;
    XORWF       dataBuffer,W    ;
    BTFSS       STATUS,Z        ;Set if we have a match
    RETURN                      ;Packet doesn't match exactly
    ;Test if we are done checking packet
    MOVF        dataIndex,W     ;Check if we have validated all 5 data bytes
    XORLW       END_OF_DATA     ;
    BTFSC       STATUS,Z        ;Z set if entire packet validated
    GOTO        PoppedConfirmed ;
    INCF        dataIndex       ;Increment dataIndex
    GOTO        PoppedTestLoop  ;Loop
PoppedConfirmed:
    ;Set output pin low since we have a popped balloon
    MOVF        PORTC,W         ;Read
...
DATA_ISR:
    ;Save new byte to dataBuffer
    MOVF        RCREG,W         ;Clear the flag by reading RCREG
    MOVWF       dataBuffer      ;Save new data byte to buffer (still in w)
    ;Check our state
...
CollectData:
    ...
    ADDWF       dataIndex,W     ;Index to our current data byte
    MOVWF       FSR             ;Set FSR to current data byte address
    MOVF        dataBuffer,W    ;Move the new data byte to W 
