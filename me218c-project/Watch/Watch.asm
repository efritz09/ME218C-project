...
;   broadcast and light LEDs upon request from the TIVA
;	
...
#define ADC_SEGMENT         0x0C	;TEAM 12 MOTHA FUCKA!
...
; Set up the ports as needed
	...
	BCF			TRISC,2			;set C2 as output for LED
	BCF			TRISC,1			;set C1 as output for LED
	BCF			TRISC,0			;set C0 as output for LED
...
Main_Loop:
    ...
    GOTO        $-1             ;No, test again, block the shit out of shit
...
BUTTON_ISR: 
    ...
    ;Testcode.. might work
    MOVF        TMR1L,W         ;Add debouncing time to current time.. does this work?
...
TIMER_ISR:
    ...
    BTFSS       debounceState,1 ;Check if we are debouncing      <------ This was wrong, changed to BTFSS which will clear debouncer
    GOTO        POP
...
RX_ISR:
    ...
    MOVWF       pairState       ;Save new data byte to buffer
