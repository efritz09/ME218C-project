...
bool Check4Keystroke(void)
{
  if ( IsNewKeyReady() ) // new key waiting?
  {
    ...
    if ( ThisEvent.EventParam == '1'){
      // Start or stop the main blower
			ES_Event NewEvent = { TOGGLE_BLOWER, 0 };
			PostMotorTester( NewEvent );
    }
		if( ThisEvent.EventParam == '2'){  
			// Start or stop the propulsion motor
			ES_Event NewEvent = { TOGGLE_PROP, 0 };
			PostMotorTester( NewEvent );	
    }
		if ( ThisEvent.EventParam == '3'){
      // Increase propeller speed
			ES_Event NewEvent = { INC_PROP, 0 };
			PostMotorTester( NewEvent );
    }
		if( ThisEvent.EventParam == '4'){  
			// Decrease propeller speed
			ES_Event NewEvent = { DEC_PROP, 0 };
			PostMotorTester( NewEvent );
		}			
		if( ThisEvent.EventParam == '5'){  
			// Increase door servo position
			ES_Event NewEvent = { INC_DOOR, 0 };
			PostMotorTester( NewEvent );	
    }
		if( ThisEvent.EventParam == '6'){  
			// Decrease door sevo position
			ES_Event NewEvent = { DEC_DOOR, 0 };
			PostMotorTester( NewEvent );	
    }
