...
Description
  Initializes the following pins:
    D0: Serial Clock    (Digital Output)
    D1: Data Line       (Digital Output)
    D2: Reset Line      (Digital Output)
****************************************************************************/
static void InitWTVPins ( void )
{
    //Initialize GPIO pins to communicate with WTV
    // Initialize the clock to GPIO Port D
    HWREG(SYSCTL_RCGCGPIO) |= SYSCTL_RCGCGPIO_R3;
    // Wait for GPIO Port D to be ready
    while ((HWREG(SYSCTL_PRGPIO) & SYSCTL_PRGPIO_R3 ) != SYSCTL_PRGPIO_R3 )
        ;
    // Enable D0-D2 as digital pins
    HWREG(GPIO_PORTD_BASE + GPIO_O_DEN) |= (CLOCK_PIN | DATA_PIN| RESET_PIN );
    // Set D0-D2 as outputs
    HWREG(GPIO_PORTD_BASE + GPIO_O_DIR) |= (CLOCK_PIN | DATA_PIN | RESET_PIN);
    // Set clock and reset to idle high
    HWREG(GPIO_PORTD_BASE+(GPIO_O_DATA + ALL_BITS))  |= (CLOCK_PIN | RESET_PIN);
    // Set data low
    HWREG(GPIO_PORTD_BASE+(GPIO_O_DATA + ALL_BITS))  &= ~DATA_PIN;
}
    ...
    if ( Event.EventType == ES_ENTRY ) {
        ...
        switch( CurrentResettingState ) {
            case ResetPulse :
                ...
                HWREG(GPIO_PORTD_BASE+(GPIO_O_DATA + ALL_BITS)) |= (RESET_PIN | CLOCK_PIN);
...
static ES_Event DuringTransferring( ES_Event Event) 
{
    ...
    if ( Event.EventType == ES_ENTRY ) {
        ...
        HWREG(GPIO_PORTD_BASE+(GPIO_O_DATA + ALL_BITS))  &= ~CLOCK_PIN;
        // 2ms delay before clock/data starts
        //ES_Timer_InitTimer(WTV_TIMER, 3);
        SetTimerMicroS(2000);
    }
    else if ( Event.EventType == ES_EXIT ) {
        // Don't think we need to do anything here
            ...
            switch( CurrentTransferringState ) {
                ...
                case SendData :
                    ...
                    if(NextCommand & DataMask)
                        HWREG(GPIO_PORTD_BASE+(GPIO_O_DATA + ALL_BITS))  |= DATA_PIN;
                    else HWREG(GPIO_PORTD_BASE+(GPIO_O_DATA + ALL_BITS))  &= ~DATA_PIN;
